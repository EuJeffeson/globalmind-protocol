            ...
            <div className="mb-8">
              ...
              <div className="space-y-4">
                {pending.map(task => {
                  const key = `${task.batchId}-${task.taskIndex}`;
                  return (
                    <div key={key} className="p-5 rounded-2xl bg-white/[0.03] border border-white/10 hover:border-[#00f5c4]/20 transition">
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-black/30 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-[#00f5c4]/50"
                          onKeyDown={e => e.key === "Enter" && submitAnswer(task)}
                        />
                        <button
                          onClick={() => submitAnswer(task)}
                          disabled={sending[key] || !answers[key]?.trim()}
                          className="px-5 py-2.5 bg-[#00f5c4] text-black font-bold rounded-xl hover:bg-[#00f5c4]/90 transition disabled:opacity-40 text-sm"
                        >
                          {sending[key] ? "..." : "Enviar"}
                        </button>
                      </div>

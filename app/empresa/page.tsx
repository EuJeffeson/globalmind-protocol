...
export default function EmpresaPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        {tasks.map((task, i) => (
          <div key={task.id} className="p-4 rounded-xl bg-white/[0.03] border border-white/10">
            ...
            <textarea
              ...
              className="w-full bg-transparent border border-white/10 rounded-lg p-3 text-sm text-white placeholder-gray-600 resize-none focus:outline-none focus:border-[#c8522a]/50 h-20"
            />
            <select
              value={task.taskType}
              onChange={e => updateTask(task.id, "taskType", Number(e.target.value))}
              className="mt-2 w-full bg-[#0a0a0a] border border-white/10 rounded-lg p-2 text-sm text-gray-300 focus:outline-none focus:border-[#c8522a]/50"
            >
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <input
            ...
            className="w-full bg-white/[0.03] border border-white/10 rounded-xl p-3 text-white font-mono focus:outline-none focus:border-[#c8522a]/50"
          />
      ...
      {/* Summary */}
      <div className="p-4 rounded-xl bg-[#c8522a]/5 border border-[#c8522a]/10 mb-6 text-sm">
        ...
        <div className="flex justify-between text-gray-400">
          ...
          <span className="text-[#c8522a] font-mono font-bold">{reward} ETH</span>
        </div>
      </div>
      ...
      <button
        onClick={submit}
        disabled={loading}
        className="w-full py-4 bg-[#c8522a] text-white font-bold rounded-xl hover:bg-[#c8522a]/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-lg"
      >
